...
def init_ui():
    display(HTML(
        ...
        "<div id='image-container' style='flex:1;border-right:2px solid #ddd;padding-right:10px;'>"
        "<img id='displayed-image' src='' style='width:100%;'></div>"
        "<div id='log-container' style='flex:1;padding-left:10px;overflow-y:auto;height:100%;border-left:2px solid #ddd;'>"
        "<pre id='log-output' style='white-space:pre-wrap;font-size:14px;'></pre></div></div>"
    ))
...
def safe_append_log(message):
    ...
    display(Javascript(
        "(function(){var e=document.getElementById('log-output');"
        "var t=document.createTextNode("+safe_message+".slice(1,-1));"
        "e.appendChild(document.createElement('br'));e.appendChild(t);e.scrollTop=e.scrollHeight;})();"
    ))
...
def update_image(path):
    ...
    display(Javascript(
        "document.getElementById('displayed-image').src='data:image/png;base64,"+img_base64+"';"
    ))
